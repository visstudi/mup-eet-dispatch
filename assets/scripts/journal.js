...
async function fetchUsersMap() {
  try {
    ...
    console.warn("Не удалось подгрузить должности пользователей");
  }
}
...
function translateAction(action, entity) {
  ...
  if (action === "Delete" || action === "remove" || action === "Remove")
    act = "Удаление";
  if (action === "Edit") act = "Изменение";
  if (action === "MarkAsRead") act = "Обработка";
  ...
  if (entity === "Vacancy") ent = "вакансии";
  if (entity === "Form") ent = "обращения";
...
window.openLogDetails = (logId) => {
  ...
  document.getElementById("log-user-role").value =
    usersMap[log.userId] || "Сотрудник";
  document.getElementById("log-time").value = formatDateTime(log.timestamp);
  document.getElementById("log-action").value = translateAction(
    log.action,
    log.entityType,
  );

  document.getElementById("log-details-text").value = formatLogDetails(
    log.entityData,
    log.entityType,
  );
