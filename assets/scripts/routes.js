...
function renderRouteList() {
    ...
    allRoutes.forEach(route => {
        ...
        art.innerHTML = `<h4>Маршрут ${route.name}</h4><p>[${route.fromStation || ''} - ${route.toStation || ''}]</p>`;
...
function initCreateOverlay() {
    ...
    overlay.querySelector('#color-input-text').oninput = (e) => updateColorPreview(e.target.value, overlay);
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            ...
            const mapUrl = overlay.querySelector('#map-link input')?.value?.trim() || "";
            ...
            const payload = {
                ...
                routeType: parseInt(overlay.querySelector('#price-variation').getAttribute('data-current-type')),
...
async function saveAllData() {
    ...
    try {
        await apiCall('/Schedule/update', 'POST', { route: currentRouteData.name, scheduleTable: currentRouteData.scheduleTable });
        ...
        const payload = {
            ...
            routeType: parseInt(panel.querySelector('#price-variation').getAttribute('data-current-type')),
        ...
        alert("Сохранено!");
        await fetchRoutes();
    } catch (e) { alert("Ошибка: " + e.message); } finally {
...
function initUI() {
    ...
    document.querySelectorAll('#price-variation .dropdown-content div').forEach(div => {
        div.onclick = () => {
            const panel = div.closest('.modify-route-info') || div.closest('.overlay');
            applyPriceLabels(div.textContent.trim() === "По способу оплаты" ? 0 : 1, panel);
            div.parentElement.style.maxHeight = null;
        };
    });
    ...
    document.getElementById('color-input-text').oninput = (e) => updateColorPreview(e.target.value, document.querySelector('.modify-route-info'));
}
...
function initOverlayWizard() {
    ...
    oTime.querySelector('form').onsubmit = (e) => {
        ...
        if (choice === "Точное время") {
            tempScheduleItem.startRange = formatTime(document.getElementById('precise-time-text').value);
