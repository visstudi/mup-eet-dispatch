...
function updateSalaryUI(typeVal, container) {
  ...
  const salaryRow =
    container.querySelector("#salary-inputs-row") ||
    container.querySelector("#job-salary");
  ...
  if (typeVal === 3) {
    ...
    if (fromField) {
      ...
      fromField.querySelector("h3").textContent =
        typeVal === 0 ? "Оклад:" : "От:";
    }
    if (toField) toField.style.display = "none";
  } else {
...
function fillData(container, data) {
  ...
  schedDrop.querySelector(".dropdown-choice").textContent = getScheduleText(
    data.workSchedule,
  );
  schedDrop.setAttribute("data-value", data.workSchedule);

  const salDrop = container.querySelector("#salary-type .dropdown");
  salDrop.querySelector(".dropdown-choice").textContent = getSalaryTypeText(
    data.salaryType,
  );
...
async function handleSave(isNew) {
  const container = isNew
    ? document.getElementById("overlay-job-create")
    : document.querySelector(".modify-job-info");
  const salType = parseInt(
    container
      .querySelector("#salary-type .dropdown")
  ...
  const payload = {
    ...
    salaryMin: sMin,
    salaryMax: sMax,
    workSchedule: parseInt(
      container
        .querySelector("#job-schedule .dropdown")
        .getAttribute("data-value"),
    ),
    requirements: container
      .querySelector("#job-requirements textarea")
      .value.trim(),
    additionalInfo: container
      .querySelector("#job-additional-info textarea")
      .value.trim(),
  };
  ...
  try {
    await apiCall(
      isNew ? "/Vacancy/create" : "/Vacancy/edit",
      isNew ? "POST" : "PATCH",
      payload,
    );
    if (isNew)
      document.getElementById("overlay-job-create").classList.add("hidden");
    alert("Сохранено!");
    await fetchJobs();
  } catch (e) {
...
function initUI() {
  ...
  document.querySelectorAll(".dropdown-content div").forEach((div) => {
    div.onclick = () => {
      ...
      if (drop.closest("#salary-type")) {
        ...
        updateSalaryUI(
          val,
          div.closest("form") || div.closest(".modify-job-info"),
        );
      }
  ...
  document.querySelector(".modify-control-buttons button:last-child").onclick =
    () => handleSave(false);
  document.querySelector("#overlay-job-create form").onsubmit = (e) => {
    e.preventDefault();
    handleSave(true);
  };

  document.querySelector(".modify-control-buttons button:first-child").onclick =
    async () => {
