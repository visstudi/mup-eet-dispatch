...
async function fetchRequests(onlyNew) {
  try {
    const data = await apiCall(
      `/Form/getAllForms?onlyNew=${onlyNew}`,
      "POST",
      {},
    );
...
function openRequestDetails(id) {
  ...
  o.querySelector("#req-type-dropdown .dropdown-choice").textContent =
    types[req.type] || "Жалоба";
  o.querySelector("#req-type-dropdown").setAttribute("data-value", req.type);
  o.querySelector("#req-status-dropdown .dropdown-choice").textContent =
    req.read ? "Обработано" : "Не обработано";
  o.querySelector("#req-status-dropdown").setAttribute(
    "data-value",
    req.read ? "true" : "false",
  );
...
async function saveRequest() {
  ...
  const typeVal =
    parseInt(
      o.querySelector("#req-type-dropdown").getAttribute("data-value"),
    ) || 0;
  const isRead =
    o.querySelector("#req-status-dropdown").getAttribute("data-value") ===
    "true";

  try {
    if (isCreateMode) {
      let digits = document
        .getElementById("req-phone")
        .value.replace(/\D/g, "");
      ...
      const timestamp = dtValue
        ? Math.floor(new Date(dtValue).getTime() / 1000)
      ...
      const payload = {
        ...
        description:
          document.getElementById("req-text").value.trim() || "Нет текста",
        smartToken: "web_admin",
      };
      await apiCall("/Form/create", "POST", payload);
    } else {
      if (isRead)
        await apiCall(`/Form/read?formId=${currentRequestId}`, "PATCH");
    }
...
function initUI() {
  document.getElementById("filter-only-new").onchange = (e) =>
    fetchRequests(e.target.checked);
  ...
  document.getElementById("btn-close-req").onclick = () =>
    document.getElementById("overlay-request-details").classList.add("hidden");
  document
    .getElementById("overlay-request-details")
    .querySelector("form").onsubmit = (e) => {
    e.preventDefault();
    saveRequest();
  };
  ...
  document.querySelectorAll(".dropdown-content div").forEach((div) => {
    div.onclick = () => {
      ...
      drop.querySelector(".dropdown-choice").textContent =
